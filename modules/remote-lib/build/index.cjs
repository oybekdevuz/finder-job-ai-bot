...
class CustomModel extends openai.ChatOpenAI {
...
agentSwarmKit.addCompletion({
    completionName: exports.CompletionName.OpenAICompletion,
    getCompletion: async ({ agentName, messages: rawMessages, mode, tools: rawTools, clientId, }) => {
        agentSwarmKit.Logger.logClient(clientId, `Using ${exports.CompletionName.OpenAICompletion} completion`, JSON.stringify(rawMessages));
...
})(exports.ToolName || (exports.ToolName = {}));

const POLICY_PROMPT = functoolsKit.str.newline([
...
const AGENT_PROMPT = functoolsKit.str.newline([
...
agentSwarmKit.addAgent({
    agentName: exports.AgentName.TriageAgentUz,
    prompt: functoolsKit.str.newline(AGENT_PROMPT),
    system: [JOB_FINDER_PROMPT, POLICY_PROMPT],
    completion: exports.CompletionName.OpenAICompletion,
    tools: [exports.ToolName.PostToChannelTool],
    callbacks: {
        onResurrect(clientId, agentName, mode, reason) {
            agentSwarmKit.Logger.logClient(clientId, "Resurrected", agentName, mode, reason);
        },
    },
});

const PARAMETER_SCHEMA = zod.z
    .object({
    message: zod.z.string().describe("Kanalga yuboriladigan post matni"),
})
    .strict();
...
agentSwarmKit.addTool({
    toolName: exports.ToolName.PostToChannelTool,
    type: "function",
    ...
    call: async ({ toolId, clientId, agentName, params }) => {
        ...
        if (!message || !message.trim()) {
            await agentSwarmKit.commitToolOutput(toolId, "Xatolik: Post matni bo'sh bo'lmasligi kerak.", clientId, agentName);
            await agentSwarmKit.execute("Foydalanuvchiga post matni bo'sh ekanligini ayting va qayta so'rang.", clientId, agentName);
            return;
        }
        if (!sendToChannel) {
            await agentSwarmKit.commitToolOutput(toolId, "Xatolik: Telegram client ulanmagan yoki kanal sozlanmagan.", clientId, agentName);
            return;
        }
        try {
            ...
            await agentSwarmKit.commitToolOutput(toolId, "Post muvaffaqiyatli kanalga joylandi.", clientId, agentName);
            await agentSwarmKit.execute("Foydalanuvchiga post muvaffaqiyatli kanalga joylanganligini xabar bering.", clientId, agentName);
        }
        catch (error) {
            console.error(`[PostToChannel] Error:`, error);
            await agentSwarmKit.commitToolOutput(toolId, `Xatolik: Kanalga post yuborishda muammo yuz berdi - ${error}`, clientId, agentName);
            await agentSwarmKit.execute("Foydalanuvchiga kanalga post yuborishda xatolik yuz berganini ayting.", clientId, agentName);
        }
    },
    function: {
...
exports.SwarmName = void 0;
(function (SwarmName) {
    SwarmName["RootSwarm"] = "root_swarm";
    SwarmName["RootSwarmEn"] = "en_swarm";
    SwarmName["RootSwarmUz"] = "uz_swarm";
    SwarmName["RootSwarmRu"] = "ru_swarm";
})(exports.SwarmName || (exports.SwarmName = {}));

agentSwarmKit.addSwarm({
    swarmName: exports.SwarmName.RootSwarm,
    agentList: [exports.AgentName.TriageAgentUz],
    defaultAgent: exports.AgentName.TriageAgentUz,
});

exports.setSendToChannel = setSendToChannel;
