...
const createCopy = (prefix = "modules") => {
    for (const modulePath of globSync(`./${prefix}/*`, { onlyDirectories: true })) {
        ...
        if (fs.existsSync(`${modulePath}/out`)) {
            fs.cpSync(`${modulePath}/out`, `${destinationPath}/out`, { recursive: true });
        }
        if (fs.existsSync(`${modulePath}/data`)) {
            fs.cpSync(`${modulePath}/data`, `${destinationPath}/data`, { recursive: true });
        }
